...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route($route . '.create') }}" class="btn btn-primary">+ Shto IPO</a>
    </div>
            ...
            <form method="GET" action="{{ route($route . '.index') }}" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="sort_by" class="form-select">
                        <option value="-public_at" {{ request('sort_by') == '-public_at' ? 'selected' : '' }}>Datë më e re</option>
                        <option value="public_at" {{ request('sort_by') == 'public_at' ? 'selected' : '' }}>Datë më e vjetër</option>
                    </select>
                </div>
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td>{{ $item->valuation_amount ? number_format($item->valuation_amount, 2) . ' ' . ($item->valuation_currency_code ?? 'USD') : '---' }}</td>
                            <td>{{ $item->raised_amount ? number_format($item->raised_amount, 2) . ' ' . ($item->raised_currency_code ?? 'USD') : '---' }}</td>
                            <td>{{ optional($item->public_at)->format('Y-m-d') ?? '---' }}</td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4 text-muted">Nuk ka IPO të regjistruara.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
