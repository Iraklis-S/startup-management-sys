...
@section('content')
    <h1 class="h3 mb-4">Përditëso IPO</h1>

    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route($route . '.update', $ipo->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name=\"company_id\" class="form-select searchable-select" data-placeholder="Kërkoni..." required>
                        ...
                        @foreach($kompanite as $kompani)
                            <option value="{{ $kompani->id }}" {{ old('company_id', $ipo->company_id) == $kompani->id ? 'selected' : '' }}>{{ $kompani->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" name="valuation_amount" class="form-control" value="{{ old('valuation_amount', $ipo->valuation_amount) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="valuation_currency_code" class="form-control" value="{{ old('valuation_currency_code', $ipo->valuation_currency_code) }}" placeholder="USD">
                ...
                <div class="row g-3 mt-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" name="raised_amount" class="form-control" value="{{ old('raised_amount', $ipo->raised_amount) }}">
                        @error('raised_amount')<div class="form-text text-danger">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Raised Currency</label>
                        <input type="text" name="raised_currency_code" class="form-control" value="{{ old('raised_currency_code', $ipo->raised_currency_code) }}" placeholder="USD">
                ...
                <div class="mb-3 mt-3">
                    ...
                    <input type="date" name="public_at" class="form-control" value="{{ old('public_at', optional($ipo->public_at)->format('Y-m-d')) }}">
                    @error('public_at')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Stock Symbol</label>
                    <input type="text" name="stock_symbol" class="form-control" value="{{ old('stock_symbol', $ipo->stock_symbol) }}">
                    @error('stock_symbol')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Source URL</label>
                    <input type="url" name="source_url" class="form-control" value="{{ old('source_url', $ipo->source_url) }}">
