...
@section('content')
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route($route . '.update', $marredhenie->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="person_id" class="form-select">
                        ...
                        @foreach($personat as $person)
                            <option value="{{ $person->company_id }}" {{ old('person_id', $marredhenie->person_id) == $person->company_id ? 'selected' : '' }}>{{ $person->first_name }} {{ $person->last_name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name=\"company_id\" class="form-select">
                        ...
                        @foreach($kompanite as $kompani)
                            <option value="{{ $kompani->id }}" {{ old('company_id', $marredhenie->company_id) == $kompani->id ? 'selected' : '' }}>{{ $kompani->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" name="start_at" class="form-control" value="{{ old('start_at', optional($marredhenie->start_at)->format('Y-m-d')) }}">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" name="end_at" class="form-control" value="{{ old('end_at', optional($marredhenie->end_at)->format('Y-m-d')) }}">
                ...
                <div class="mb-3 form-check">
                    ...
                    <input type="checkbox" name="is_past" value="1" class="form-check-input" id="is_past" {{ old('is_past', $marredhenie->is_past) ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_past">Historike</label>
                </div>

                <div class="mb-3">
                    <label class="form-label">Sequence</label>
                    <input type="number" name="sequence" class="form-control" value="{{ old('sequence', $marredhenie->sequence) }}">
                    @error('sequence')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Title</label>
                    <input type="text" name="title" class="form-control" value="{{ old('title', $marredhenie->title) }}">
