...
@section('content')
    <h1 class="h3 mb-4">Përditëso Arritje</h1>

    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route($route . '.update', $arritje->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name=\"company_id\" class="form-select searchable-select" data-placeholder="Kërkoni..." required>
                        ...
                        @foreach($kompanite as $kompani)
                            <option value="{{ $kompani->id }}" {{ old('company_id', $arritje->company_id) == $kompani->id ? 'selected' : '' }}>{{ $kompani->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="milestone_at" class="form-control" value="{{ old('milestone_at', optional($arritje->milestone_at)->format('Y-m-d')) }}">
                    @error('milestone_at')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Kodi i Milestone</label>
                    <input type="text" name="milestone_code" class="form-control" value="{{ old('milestone_code', $arritje->milestone_code) }}">
                    @error('milestone_code')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">URL Burim</label>
                    <input type="url" name="source_url" class="form-control" value="{{ old('source_url', $arritje->source_url) }}">
                    @error('source_url')<div class="form-text text-danger">{{ $message }}</div>@enderror
                </div>

                <div class="mb-3">
                    <label class="form-label">Përshkrim Burim</label>
                    <textarea name="source_description" class="form-control" rows="3">{{ old('source_description', $arritje->source_description) }}</textarea>
